...
function verifyLabel(v) {
  return { unverified: "직접 입력", screenshot: "인증됨", suspected: "의심 중", mismatch: "불일치" }[v] || "";
}
...
function holdingRow(h, isMine) {
  ...
  if (isMine && h.status === "holding") {
    actions = sm("추천", `open-rec:${h.id}`) + sm("같이 사기", `open-prop:${h.stockId}`) + sm("매도", `sell:${h.id}`);
    if (h.verification !== "screenshot") actions += sm("인증", `verify:${h.id}`);
  } else if (!isMine && h.status === "holding") {
    actions = sm("구라핑 의심", `suspect:${h.id}`);
  }
  ...
  return `
    <div class="row">
      ...
      <div class="right">
        ...
        <div class="status">${verifyLabel(h.verification)}</div>
      </div>
    </div>`;
}
...
function inboxBlock(item) {
  if (item.kind === "recommend") {
    ...
    return `<div class="action-block">
      <h4>${esc(nickname(rec.senderId))} · ${esc(s.name)} 추천</h4>
      ${h ? `<p>${esc(nickname(rec.senderId))} 평단 ${formatPrice(h.averagePrice, s.market)} · ${formatPct(ret(h.averagePrice, priceOf(s)))}</p>` : ""}
      <p>${esc(rec.message)}</p>
      <div class="split">${btn("나도 추가", "primary", `accept:${rec.id}`)}${btn("나중에", "secondary", `reject:${rec.id}`)}</div>
    </div>`;
  }
  if (item.kind === "proposal" || item.kind === "nag") {
    const p = item.proposal;
    const title = item.kind === "nag"
      ? `${esc(nickname(p.proposerId))} · 같이 사자고 조르는 중`
      : `${esc(nickname(p.proposerId))} · 같이 사기 제안`;
    return `<div class="action-block">
      <h4>${title}</h4>
      <p>${esc(p.message)}</p>
      <div class="split">${btn("같이 사기", "primary", `promise:${p.id}`)}${btn("나중에", "secondary", `later:${p.id}`)}</div>
    </div>`;
  }
  if (item.kind === "cobuyRegister") {
    ...
    return `<div class="action-block">
      <h4>약속만 하고 아직 미등록</h4>
      <p>${esc(s.name)}를 등록해야 깐부가 됩니다.</p>
      ${btn("지금 등록", "primary", `register:${s.id}`)}
    </div>`;
  }
  if (item.kind === "suspect") {
    ...
    const s = stock(h.stockId);
    return `<div class="action-block">
      <h4>${esc(s.name)} 매수가 의심</h4>
      <p>${formatPrice(h.averagePrice, s.market)}에 산 기록이 맞는지 확인해 주세요. 사기라고 단정하지 않습니다.</p>
...
function renderActivity() {
  ...
  return `<div class="screen">
    ...
    <p class="page-sub">나를 향한 추천과 제안</p>
...
function sheetHTML() {
  ...
  if (state.sheet.startsWith("add") || state.sheet.startsWith("register:")) {
    ...
    return `<div class="sheet" data-act="close"><div class="panel" onclick="event.stopPropagation()">
      <h2>주식 추가</h2>
      <p class="note">같이 사기 약속만으로는 깐부가 되지 않습니다.</p>
      <label>종목</label>
  ...
  if (state.sheet.startsWith("open-rec:")) {
    ...
    return `<div class="sheet" data-act="close"><div class="panel" onclick="event.stopPropagation()">
      <h2>추천하기</h2>
      <p class="note">선택한 친구에게 이 종목을 추천합니다.</p>
